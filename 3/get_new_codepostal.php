<?php
	...
	function n_getCodePostalData($codePostal) {
		...
		fwrite($file_, "3\n");
				...
				$root = substr($codePostal,0,3); //r3b
				$fichierCP = "codePostal/$root.cp"; // codePostal/r3b.cp

				if (file_exists("$fichierCP")) { //Oui
		fwrite($file_, "4\n");
		...
		fwrite($file_, "6\n");
							...
							if (mysql_num_rows($res) == 0) {
		fwrite($file_, "7\n");
								//Debug here  Reda 26/02/2020
		fwrite($file_, $codePostal."\n");
								$json   = file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?address='.urlencode($codePostal).'&region=ca');
								$output = json_decode($json);
		fwrite($file_, $output ."\n");
								$lat    = $output->results[0]->geometry->location->lat;
								$lng    = $output->results[0]->geometry->location->lng;
		fwrite($file_, "lat = ".$lat."\nlon = ".$lng."\n");
