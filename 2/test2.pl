...
while (my $ligne = <IN>){
  ...
  my $select = "SELECT * FROM $database_produit WHERE $tab[ID_DATABASE]=id_database AND date='$tab[DATE]' AND id_enseigne=$tab[ID_ENSEIGNE] AND prix_special=$tab[PRIX_SPECIAL] AND ligne_complete_es LIKE '\%soscuisine\%' AND id_categorie=$tab[ID_CATEGORIE]";
