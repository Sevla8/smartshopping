			...
			if (mysql_num_rows($res) > 0){
				...
				if(file_exists("$fichierCP")){
					...
					while (!feof($fp)) { //on parcourt toutes les lignes
						...
						if ($cp == $codePostal){
                            ...
                            if(mysql_num_rows($res) == 0){
				//	$file_ = fopen('trace0.txt', 'w');
								//$json   = file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?address='.urlencode($codePostal).'&region=ca&&key=AIzaSyDuTDLzojQ5XPa2SYanMJbJ_72yuwiPxgU');
				...
				//	fclose($file_);
      					...
      					$latitude   = $lat;
                                ...
                                $res   = mysql_query($query);
                                if(mysql_num_rows($res) == 0){
                                	$query = "INSERT INTO Poste_Canada VALUES('$codePostal',$latitude,$longitude,'$ville','$province',$valide,$valide,0)";
                                    mysql_query($query);
                                }
                                else{
